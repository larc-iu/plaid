...
from plaid_igt_agent.conversation import (
...
def test_store_round_trip_and_ownership():
    c = FakeClient()
    store = ConversationStore(c, 'u@x', 'p1')
    try:
        store.load('c1')
    ...
    assert set(c.user_data.store) == {('u@x', conv_key('p1', 'c1')), ('u@x', meta_key('p1', 'c1'))}
    ...
    c.user_data.delete('u@x', meta_key('p1', 'c1'))
