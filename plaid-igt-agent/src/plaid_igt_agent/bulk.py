...
from .tools import Workspace, ToolError, t_set_analysis, entry_line
from .stats import _analyzed, _docs, _tag

MAX_BULK = 3000


def _replacer(pattern: str, replacement: str, regex: bool, whole_value: bool):
    if not pattern:
    ...
    if regex:
        ...
        return lambda v: rx.sub(replacement, v)
    if whole_value:
        return lambda v: replacement if v == pattern else v
    return lambda v: v.replace(pattern, replacement)
...
def t_replace_in_field(ws: Workspace, field: str, pattern: str, replacement: str, regex: bool = False,
                       whole_value: bool = False, document: Optional[str] = None) -> str:
    """PLAN: substitute inside every value of a field (substring, whole value,
    or regex with backreferences), project-wide or in one document."""
    f = ws.project.field(field)
    rep = _replacer(pattern, replacement, bool(regex), bool(whole_value))
    labels: List[str] = []
    for doc in _docs(ws, document):
        for s in doc.sentences:
            ...
            for u, ref, what in units:
                ...
                if new == sp.value:
                    continue
                label = f'{doc.name} {ref} "{what[:30]}": {f.name} "{sp.value}" → "{new}"' + (' (cleared)' if new == '' else '')
                ws.add_op({'kind': 'set_span', 'layer_id': f.layer_id, 'token_id': u.id, 'span_id': sp.id,
                           'value': new, 'label': label})
                labels.append(label)
    _check_cap(len(labels))
...
def t_respell_all(ws: Workspace, pattern: str, replacement: str, regex: bool = False,
                  whole_word: bool = False, document: Optional[str] = None) -> str:
    ...
    word boundaries."""
    rep = _replacer(pattern, replacement, bool(regex), bool(whole_word))
    labels: List[str] = []
    for doc in _docs(ws, document):
        for s in doc.sentences:
            for w in s.words:
                ...
                ws.add_op({'kind': 'respell', 'text_id': w.text_id, 'begin': w.begin, 'end': w.end, 'value': new,
                           'label': label})
                labels.append(label)
    _check_cap(len(labels))
    ...
    for doc in _docs(ws, document):
        for s in doc.sentences:
            for w in s.words:
                ...
                ws.add_op({'kind': 'set_orthography', 'word_id': w.id, 'key': f'orthog:{target}', 'value': value, 'label': label})
                labels.append(label)
    _check_cap(len(labels))
    return _bulk_note(ws, len(labels), labels, 'words')
        ...
        for s in doc.sentences:
            for w in s.words:
                ...
                if len(ws.ops) > before:
                    n += 1
                    labels.append(ws.ops[-1]['label'])
                    if not first_note and 'differ from the surface' in note:
                        first_note = note[note.index('(note'):]
    _check_cap(n)
    return _bulk_note(ws, n, labels, f'occurrences of "{form}"') + (' ' + first_note if first_note else '')
