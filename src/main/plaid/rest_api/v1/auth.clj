  ...
  ["/login"
   {:post {:summary    "Authenticate a user and get a JWT token"
           ...
           :handler    (fn [{{{:keys [username password]} :body} :parameters xtdb :xtdb secret-key :secret-key}]
                         (if-let [{:user/keys [id password-changes password-hash]} (pxu/get xtdb username)]
      ...
      (cond (nil? secret-key)
            ...
            (let [token (subs auth-header 7)
                  token-data (try (jwt/unsign token secret-key)
                                  (catch Exception e e))
                  user (and (map? token-data) (pxu/get xtdb (:user/id token-data)))]
              (cond
...
(defn wrap-admin-required [handler]
  (fn [{:keys [xtdb] :as request}]
    (let [{:user/keys [is-admin]} (pxu/get xtdb (-> request :jwt-data :user/id))]
      (if-not is-admin
        {:status 403
