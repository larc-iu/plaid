...
(defn layer-config-routes [id-keyword]
  ["/config/:namespace/:config-key"
   {:middleware [[pra/wrap-writer-required #(-> % :parameters :path id-keyword)]]
    :put        {:summary    (str "Set a configuration value for a layer in a editor namespace. "
                                  "Good for storing data useful for ")
    ...
    :delete     {:summary    "Remove a configuration value for a layer in a editor namespace"
