...
(defn- project-id-from-sl [db-like id]
  (-> (xt/q (pxc/->db db-like)
            '{:find  [?prj]
              :where [[?prj :project/text-layers ?txtl]
                      [?txtl :text-layer/token-layers ?tokl]
...
(defn create-operation
  ...
  (let [{:keys [db]} (pxc/ensure-db xt-map)
        ...
        project-id (project-id-from-sl db span-layer-id)
...
(defn merge-operation
  ...
  (let [{:keys [db]} (pxc/ensure-db xt-map)
        ...
        tx-ops (do (when-let [name (:relation-layer/name m)]
                     (pxc/valid-name? name))
                   (pxc/merge* xt-map :relation-layer/id eid (select-keys m [:relation-layer/name])))]
    (op/make-operation
