"""``plaid_agent.igt.vocab`` against the app's own domain modules, function by
...
def test_every_app_function_is_ported_or_exempted():
    ...
    assert not extra, (
        f'plaid_agent/igt/vocab.py has public functions the app does not export: {extra}. '
    ...
    for i, (a, b) in enumerate(zip(js, py)):
        if a[key] != b[key]:
            pytest.fail(
                ...
                'Update plaid_agent/igt/vocab.py to match, then check what reads it: a change to '
