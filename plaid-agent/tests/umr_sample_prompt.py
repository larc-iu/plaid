"""Write docs/umr/SAMPLE_PROMPT.md: what the model sees, rendered for a reader.

The system prompt and the tool list are built by the real code over the small
project the tests use (``umr_fixtures.py``), with web lookup on so every tool
...
from umr_fixtures import PID, umr_client  # noqa: E402

from plaid_agent.umr.project import load_project  # noqa: E402
from plaid_agent.umr.prompt import build_system_prompt  # noqa: E402
from plaid_agent.umr.toolkit import TOOLS, WEB_TOOLS, WRITE_TOOLS, call_tool  # noqa: E402
from plaid_agent.umr.tools import Workspace  # noqa: E402

OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'docs', 'umr',
                   'SAMPLE_PROMPT.md')
...
def _render() -> str:
    ...
    parts = [HEADER, '\n## System prompt\n', '```text',
             ...
             f'## Tools\n\n{len(TOOLS)} tools, in the order the model receives them: '
             f'{len(WRITE_TOOLS)} plan a change (`PLAN:`), {len(WEB_TOOLS)} reach the web, the '
             f'rest read the project or manage the plan.\n']
    parts += [render_tool(t, WEB_TOOLS) for t in TOOLS]
    parts += ['## What a read returns\n',
              'Two tool results on the same project, so the positional addressing in the prompt '
              'has something to point at. `project_overview` is what the prompt tells the model '
              'to call first.\n',
              '```text', call_tool(ws, 'project_overview', {}).rstrip(), '```\n',
              '`read_document` on "Story":\n',
              '```text', call_tool(ws, 'read_document', {'document': 'Story'}).rstrip(), '```\n']
    return '\n'.join(parts)
