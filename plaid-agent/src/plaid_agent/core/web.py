...
FETCH_TIMEOUT_S = 20
MAX_REDIRECTS = 5
# What a fetched site sees. One name for every app's assistant: a site that
...
def fetch(url: str, cfg: WebConfig, client=None) -> tuple:
    ...
    try:
        for _ in range(MAX_REDIRECTS + 1):
            try:
                with client.stream('GET', seen, headers={'User-Agent': USER_AGENT}) as r:
                    if r.is_redirect:
...
def ping(cfg: WebConfig, query: str = 'linguistic annotation') -> int:
