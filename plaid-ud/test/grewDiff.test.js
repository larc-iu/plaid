...
test('column and FEATS changes become span writes with the old metadata', () => {
  ...
  assert.equal(upos.id, r.before.nodes.get(r.before.order[2]).spanIds.upos);
...
test('del_node deletes the token (or just the word of a multi-word token) and nothing under it', () => {
  ...
  const perro = r.before.nodes.get(r.before.order[2]);
  ...
  assert.deepEqual(r.writes.tokens, [{ op: 'deleteToken', id: r.before.order[1] }]);
});
