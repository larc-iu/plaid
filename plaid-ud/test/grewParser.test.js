...
test('ordering, dominance (plain + labeled), crossing', () => {
  ...
  assert.deepEqual(items('pattern { e1 >< e2 }')[0], {
    kind: 'cross',
    left: 'e1',
    right: 'e2',
    line: 1,
...
test('cross-node feature comparison vs single-node feature constraint', () => {
  assert.deepEqual(items('with { X.lemma = Y.lemma }')[0], {
    kind: 'featcmp',
    left: { node: 'X', feat: 'lemma' },
    op: '=',
    right: { node: 'Y', feat: 'lemma' },
    line: 1,
  });
...
test('global flags and metadata', () => {
  assert.deepEqual(parse('global { is_projective }').blocks[0].items[0], {
    kind: 'globalflag',
    name: 'is_projective',
    line: 1,
  });
...
test('multiple blocks', () => {
  ...
  assert.deepEqual(
    ...
    ['pattern', 'without', 'global'],
  );
});

test('parse errors carry location', () => {
  assert.throws(
    () => parse('pattern { X [upos=] }'),
    (e) => e.name === 'GrewParseError' && e.line === 1,
  );
  assert.throws(
    () => parse('foo { X }'),
    (e) => /block keyword/.test(e.message),
  );
});
