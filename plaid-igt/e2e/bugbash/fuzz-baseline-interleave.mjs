...
function isInfraError(s) {
  return /Database busy|Internal error|HTTP 50\d|HTTP 503|Service Unavailable/i.test(
    String(s || ''),
  );
}
...
async function runOp(doc, label, fn) {
  ...
  try {
    ...
    if (isInfraError(e && (e.message || e))) {
      infraErrors++;
      return { ok: false, threw: true, infra: true, err: String(e) };
    }
    record({
      kind: 'uncaught-exception',
      title: `${label} THREW instead of returning false`,
...
async function buildUp(doc, n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const te = t + 1;
    t += randInt(rng, 1, 2);
    await runOp(doc, 'createAlignment(buildup)', () =>
      doc.createAlignment({ text, timeBegin: tb, timeEnd: te }),
    );
  }
}
...
async function scenarioBuildThenSave(client, projectId, idx) {
  ...
  try {
    ...
    if (doc.isSaving)
      record({
        kind: 'uncaught-exception',
        title: 'isSaving stuck true after saveBaselineText',
        detail: `target="${target}"`,
        ctx: 'wedged',
      });

    const fresh = await reloadFresh(client, projectId, documentId);
    const { issues, wordsN, morphN, sentsN, alignsN, len } = checkSavedState(
      fresh,
      r.ok ? target : null,
      r.ok,
    );
    ...
    if (!r.ok && !r.threw && !r.infra) {
      ...
      record({
        kind: 'false-rejection',
        title: `saveBaselineText("${target}") returned false`,
        detail: `seed="${seedBody}" build=${nBuild} err="${r.err}"`,
        ctx: 'save-false',
      });
    }
    for (const iss of issues) {
      record({
        ...
        title: `post-save violation (seed="${seedBody}" target="${target}")`,
        detail: iss,
        ctx: `seed=${seedBody}|build=${nBuild}|target=${target}`,
      });
    }
...
async function scenarioEmptyThenNonEmpty(client, projectId) {
  ...
  try {
    ...
    log(
      `[empty->] save("") ret=${r1.ok}${r1.infra ? ' (infra-noise)' : ''} body="${fresh1.body}" sents=${c1.sentsN} aligns=${c1.alignsN} textObjGone=${textGone}`,
    );
    for (const iss of c1.issues)
      record({
        kind: 'invariant-violation',
        title: 'after saveBaselineText("")',
        detail: iss,
        ctx: 'empty',
      });
    if (!r1.ok && !r1.threw && !r1.infra)
      record({
        kind: 'false-rejection',
        title: 'saveBaselineText("") returned false',
        detail: `err="${r1.err}"`,
        ctx: 'empty',
      });

    // This is the "no existing text" branch IF emptying removed the text object.
    const r2 = await runOp(doc, 'saveBaselineText(nonempty after empty)', () =>
      doc.saveBaselineText('reborn baseline text'),
    );
    ...
    log(
      `[->nonempty] save("reborn...") ret=${r2.ok}${r2.infra ? ' (infra-noise)' : ''}${r2.err ? ` err="${r2.err}"` : ''} body="${fresh2.body}" sents=${c2.sentsN} aligns=${c2.alignsN} (elseBranchUsed=${textGone})`,
    );
    for (const iss of c2.issues)
      record({
        kind: 'invariant-violation',
        title: 'after empty->nonempty (else/rollback branch)',
        detail: iss,
        ctx: 'reborn',
      });
    if (!r2.ok && !r2.threw && !r2.infra)
      record({
        kind: 'false-rejection',
        title: 'saveBaselineText after empty returned false',
        detail: `err="${r2.err}" elseBranch=${textGone}`,
        ctx: 'reborn',
      });
  } finally {
    await cleanupDoc(client, documentId);
...
async function scenarioSameText(client, projectId) {
  ...
  try {
    ...
    log(
      `[same] save(same) ret=${r.ok}${r.infra ? ' (infra-noise)' : ''} body="${fresh.body}" sents=${c.sentsN} words=${beforeWords}->${c.wordsN} morph=${beforeMorph}->${c.morphN}`,
    );
    for (const iss of c.issues)
      record({
        kind: 'invariant-violation',
        title: 'after saveBaselineText(same text)',
        detail: iss,
        ctx: 'same',
      });
    if (!r.ok && !r.threw && !r.infra)
      record({
        kind: 'false-rejection',
        title: 'saveBaselineText(same text) returned false',
        detail: `err="${r.err}"`,
        ctx: 'same',
      });
    // NEW CONTRACT: an identical body is a no-op diff — every token survives.
    if (r.ok) {
      if (c.wordsN !== beforeWords)
        record({
          kind: 'data-corruption',
          title: 'same-text save changed word count',
          detail: `${beforeWords} -> ${c.wordsN}`,
          ctx: 'same',
        });
      if (c.morphN !== beforeMorph)
        record({
          kind: 'data-corruption',
          title: 'same-text save changed morpheme count',
          detail: `${beforeMorph} -> ${c.morphN}`,
          ctx: 'same',
        });
    }
  } finally {
...
async function scenarioAppendPreserves(client, projectId) {
  const { doc, documentId } = await freshDoc(client, projectId, {
    body: 'palabras que deben sobrevivir',
  });
  docsUsed++;
  try {
    await buildUp(doc, 2);
    ...
    log(
      `[append] ret=${r.ok}${r.infra ? ' (infra-noise)' : ''}${r.err ? ` err="${r.err}"` : ''} words=${beforeWords}->${c.wordsN} morph=${beforeMorph}->${c.morphN} aligns=${beforeAligns}->${c.alignsN}`,
    );
    for (const iss of c.issues)
      record({
        kind: 'invariant-violation',
        title: 'after append-only save',
        detail: iss,
        ctx: 'append',
      });
    if (!r.ok && !r.threw && !r.infra)
      record({
        kind: 'false-rejection',
        title: 'append-only saveBaselineText returned false',
        detail: `err="${r.err}"`,
        ctx: 'append',
      });
    if (r.ok) {
      if (c.wordsN !== beforeWords)
        record({
          kind: 'data-corruption',
          title: 'append-only save changed word count',
          detail: `${beforeWords} -> ${c.wordsN}`,
          ctx: 'append',
        });
      if (c.morphN !== beforeMorph)
        record({
          kind: 'data-corruption',
          title: 'append-only save changed morpheme count',
          detail: `${beforeMorph} -> ${c.morphN}`,
          ctx: 'append',
        });
      if (c.alignsN !== beforeAligns)
        record({
          kind: 'data-corruption',
          title: 'append-only save changed alignment count',
          detail: `${beforeAligns} -> ${c.alignsN}`,
          ctx: 'append',
        });
    }
  } finally {
...
async function scenarioRapidBackToBack(client, projectId) {
  ...
  try {
    ...
    try {
      ...
      record({
        kind: 'uncaught-exception',
        title: 'rapid saveBaselineText threw synchronously',
        detail: String(e),
      });
    }
    ...
    if (doc.isSaving)
      record({
        kind: 'uncaught-exception',
        title: 'isSaving stuck after rapid saves',
        ctx: 'rapid',
      });
    if (r1 === true && r2 === true) {
      record({
        kind: 'false-acceptance',
        title: 'both rapid saves returned true (single-flight broken)',
        detail: 'expected second to no-op (false)',
        ctx: 'rapid',
      });
    }
    const fresh = await reloadFresh(client, projectId, documentId);
    ...
    if (r1 && !r2 && !infra && fresh.body !== expectWinner) {
      record({
        kind: 'data-corruption',
        title: 'rapid: single-flight held but body != first save',
        detail: `body="${fresh.body}" expected="${expectWinner}"`,
        ctx: 'rapid',
      });
    }
    ...
    for (const iss of c.issues)
      record({
        kind: 'invariant-violation',
        title: 'after rapid back-to-back saves',
        detail: iss,
        ctx: 'rapid',
      });
  } finally {
...
async function scenarioEmptyThenNonEmptyVariants(client, projectId) {
  ...
  try {
    ...
    for (const target of seq) {
      const r = await runOp(doc, `saveBaselineText("${target}")`, () =>
        doc.saveBaselineText(target),
      );
      const fresh = await reloadFresh(client, projectId, documentId);
      const c = checkSavedState(fresh, r.ok ? target : null, r.ok);
      log(
        `[seq] save("${target}") ret=${r.ok}${r.infra ? ' (infra-noise)' : ''}${r.err ? ` err="${r.err}"` : ''} body="${fresh.body}" sents=${c.sentsN}`,
      );
      for (const iss of c.issues)
        record({
          kind: 'invariant-violation',
          title: `empty/nonempty sequence step "${target}"`,
          detail: iss,
          ctx: `seq:${target}`,
        });
      if (!r.ok && !r.threw && !r.infra)
        record({
          kind: 'false-rejection',
          title: `sequence saveBaselineText("${target}") returned false`,
          detail: `err="${r.err}"`,
          ctx: `seq:${target}`,
        });
    }
...
async function main() {
  ...
  await scenarioEmptyThenNonEmpty(client, projectId);
  await sleep(150);
  await scenarioEmptyThenNonEmptyVariants(client, projectId);
  await sleep(150);
  await scenarioSameText(client, projectId);
  await sleep(150);
  await scenarioAppendPreserves(client, projectId);
  await sleep(150);
  await scenarioRapidBackToBack(client, projectId);
  await sleep(150);
  ...
  for (let i = 0; i < N && docsUsed < 36; i++) {
    try {
      ...
      if (isInfraError(e && (e.message || e))) {
        infraErrors++;
        log(`[s${i}] skipped (infra: ${e.message || e})`);
      } else
        record({
          kind: 'uncaught-exception',
          title: `scenario ${i} crashed`,
          detail: String((e && e.stack) || e),
        });
    }
    await sleep(120);
  ...
  for (const f of findings)
    log(`  - [${f.kind}] ${f.title}${f.detail ? ` :: ${f.detail.split('\n')[0]}` : ''}`);
}
