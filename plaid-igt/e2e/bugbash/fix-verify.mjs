...
// 1 + 2: the headline false-rejections are gone
{
  ...
  try {
    ...
    check(
      '  -> invariants clean',
      runAllInvariants(fresh).violations.length === 0,
      JSON.stringify(runAllInvariants(fresh).violations),
    );
  } finally {
...
// 3: negative duration rejected (no server write)
{
  ...
  try {
    ...
    check(
      'createAlignment negative-duration rejected',
      ok === false && /Invalid time range/.test(doc.error || ''),
    );
  } finally {
    await cleanupDoc(client, documentId);
  }
...
// 4 + 5: temporal-inversion guard on updateAlignmentBounds + valid edit still works
{
  ...
  try {
    ...
    const first = doc.layerInfo.alignmentTokenLayer.tokens
      .slice()
      .sort((a, b) => a.begin - b.begin)[0].id;
    const bad = await doc.updateAlignmentBounds(first, { timeBegin: 100, timeEnd: 101 }); // would invert vs 'cat'
    check(
      'updateAlignmentBounds inversion rejected',
      bad === false && /temporal order/.test(doc.error || ''),
      `err="${doc.error}"`,
    );
