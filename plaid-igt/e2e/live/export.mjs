...
// flow as import-flex.mjs), then drives runExport — plain text + FLEx
...
const setupData = {
  ...
  vocabulary: {
    vocabularies: [
      { id: 'new-flex', name: `${projectName} Lexicon`, enabled: true, isCustom: true },
    ],
  },
  documentMetadata: {
...
try {
  ...
  check(
    layers.orthographies.includes('Translit'),
    'discovered Translit orthography',
  ...
  check(
    layers.sentFields.some((f) => /translation/i.test(f)),
    'discovered sentence translation field',
    JSON.stringify(layers),
  );
  ...
  const plain = await runExport({
    ...
    onProgress: ({ done, total, name }) =>
      name && console.log(`  plain ${done + 1}/${total} ${name}`),
  });
  ...
  check(
    plainDocs.every((p) => p.endsWith('.txt')),
    'plain entries end in .txt',
    JSON.stringify(plainDocs),
  );
  ...
  check(
    tsvLines.length === (vocab.items?.length ?? 0) + 1,
    `vocab TSV rows = items + header (${(vocab.items?.length ?? 0) + 1})`,
    `got ${tsvLines.length}`,
  );
  ...
  const flex = await runExport({
    ...
    scope: { type: 'project' },
    onProgress: ({ done, total, name }) =>
      name && console.log(`  flex ${done + 1}/${total} ${name}`),
  });
  ...
  const native = await runExport({
    ...
    scope: { type: 'project' },
    onProgress: ({ done, total, name }) =>
      name && console.log(`  native ${done + 1}/${total} ${name}`),
  });
  ...
  for (const [path, bytes] of Object.entries(nativeEntries)) {
    ...
    try {
      nativeJson[path] = JSON.parse(decode(bytes));
    } catch (e) {
      parseFailed = `${path}: ${e.message}`;
    }
  }
  ...
  check(
    manifest?.format === 'plaid-igt' && manifest?.formatVersion === 1,
    'manifest format/version',
    JSON.stringify({ format: manifest?.format, formatVersion: manifest?.formatVersion }),
  );
  check(
    manifest.documents.length === 3,
    'manifest lists 3 documents',
    String(manifest.documents.length),
  );
  check(
    manifest.documents.every((d) => nativeJson[d.file]),
    'manifest files all exist in the zip',
  );
  check(
    manifest.schema.orthographies.some((o) => o.name === 'Translit'),
    'schema lists Translit',
  ...
  check(
    vocabJson?.items?.length === (vocab.items?.length ?? 0),
    `native vocab has ${vocab.items?.length} items`,
    String(vocabJson?.items?.length),
  );
  const idsAscending = vocabJson.items.every(
    (it, i) => i === 0 || vocabJson.items[i - 1].id < it.id,
  );
  ...
  check(
    someWord.morphemes.every((m, i) => m.precedence === i + 1),
    'morpheme precedence is 1-based in order',
    JSON.stringify(someWord.morphemes.map((m) => m.precedence)),
  );
  const itemIds = new Set(vocabJson.items.map((it) => it.id));
  const linkedMorph = docJson.sentences
    .flatMap((s) => s.words)
    .flatMap((w) => w.morphemes)
    .find((m) => m.vocab);
  check(
    !!linkedMorph && itemIds.has(linkedMorph.vocab.itemId),
    'a morpheme vocab link resolves to a vocab item',
    JSON.stringify(linkedMorph?.vocab),
  );
  check(
    typeof docJson.baseline.body === 'string' && docJson.baseline.body.length > 0,
    'native doc carries the baseline body',
  );
  check(
    docJson.metadata && typeof docJson.metadata === 'object',
    'native doc carries raw metadata',
  );
