...
test('deleting a segment takes its text, and existing text is aligned by selecting it', async ({
  page,
}) => {
  const diag = collectClientErrors(page);
  await seedAuth(page);
  ...
  // A segment is its utterance: with nothing annotated on it, the trash takes
  // the row and its text at once, with no dialog. (An annotated one asks in
  // place; TranscriptList.test.jsx covers that.)
  ...
  const bodyAfter = bodyBefore.replace(/\s*hello there friend\s*/, '');
  await expect.poll(bodyNow).toBe(bodyAfter);
  ...
  await expect(existing).toHaveValue(bodyAfter);
