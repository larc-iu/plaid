...
const wl = tl.tokenLayers.find((l) => roleOf(l) === ROLES.WORD),
  ml = tl.tokenLayers.find((l) => roleOf(l) === ROLES.MORPHEME);
const gloss = ml.spanLayers.find((s) => s.name === 'Gloss'),
  pos = wl.spanLayers.find((s) => s.name === 'Part of Speech');
const words = [...wl.tokens]
...
out.ser2 = (
  await c.spans.create(
    ...
    [W('ser', 1).morphs[0].id],
    'VER',
    confirmedInferred('service:test'),
  )
).id;
out.hola = (
  await c.spans.create(gloss.id, [W('"hola,"').morphs[0].id], 'HI', stampInferred('service:test'))
).id;
...
out.ko = ko.id;
out.koGloss = (
  await c.spans.create(gloss.id, [ko.id], 'PST', stampInferred('rule:analysis-precedent'))
).id;
out.ngoLink = (
  await c.vocabLinks.create(
    item('the').id,
    [ng.morphs[0].id],
