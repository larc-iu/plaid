...
export const tokenMutations = {
  ...
  // Capture everything deleteToken(tokenId) will destroy — the word token,
  // its coincident morphemes, spans pinned to any of them, and their vocab
  // links — so the deletion can be undone. Returns null if the token isn't a
  // word token. Pure read; safe to call right before deleteToken.
  snapshotWordToken(tokenId) {
    const info = this.layerInfo;
    const word = (info.primaryTokenLayer?.tokens || []).find(t => t.id === tokenId);
    const textId = info.primaryTextLayer?.text?.id;
    if (!word || !textId) return null;
    ...
    const deadIds = new Set([word.id, ...morphemes.map(m => m.id)]);

    const spans = [];
    for (const scope of ['word', 'morpheme']) {
      for (const sl of info.spanLayers?.[scope] || []) {
        for (const s of sl.spans || []) {
          if (Array.isArray(s.tokens) && s.tokens.some(t => deadIds.has(t))) {
            spans.push({ layerId: sl.id, tokens: [...s.tokens], value: s.value, metadata: s.metadata });
          }
        }
      }
    }
    ...
    for (const vocab of Object.values(this._vocabularies || {})) {
      for (const link of vocab.vocabLinks || []) {
        if (Array.isArray(link.tokens) && link.tokens.some(t => deadIds.has(t)) && link.vocabItem?.id) {
          vocabLinks.push({ vocabItemId: link.vocabItem.id, tokens: [...link.tokens] });
        }
      }
    }
    ...
    return {
      textId,
      word: { id: word.id, begin: word.begin, end: word.end, metadata: word.metadata || {} },
      morphemes,
      spans,
      vocabLinks
    };
  ...
  // Undo a word-token deletion from a snapshotWordToken capture: recreate the
  // word, its morphemes, and their spans/vocab links (dead ids remapped onto
  // the freshly created ones). Sequential creates — later entities need the
  // ids the earlier calls return. Fails loudly (server rejects out-of-bounds
  // offsets) if the text changed since the snapshot.
  async restoreWordToken(snapshot) {
    if (!snapshot) return false;
    return this._withSaving('Failed to restore token', async () => {
      const info = this.layerInfo;
      const wordLayerId = info.primaryTokenLayer?.id;
      const morphemeLayerId = info.morphemeTokenLayer?.id;
      if (!wordLayerId) throw new Error('No word token layer found');

      // The client only omits `undefined` body fields — a `null` precedence or
      // an empty metadata map would go over the wire and be rejected.
      const orOmit = (v) => (v == null ? undefined : v);
      const metaOrOmit = (m) => (m && Object.keys(m).length > 0 ? m : undefined);

      const idMap = new Map();
      const newWord = await this._client.tokens.create(
        wordLayerId, snapshot.textId, snapshot.word.begin, snapshot.word.end, undefined, metaOrOmit(snapshot.word.metadata));
      idMap.set(snapshot.word.id, newWord.id);

      for (const m of snapshot.morphemes) {
        if (!morphemeLayerId) break;
        const newMorph = await this._client.tokens.create(
          morphemeLayerId, snapshot.textId, m.begin, m.end, orOmit(m.precedence), metaOrOmit(m.metadata));
        idMap.set(m.id, newMorph.id);
      }

      for (const s of snapshot.spans) {
        const tokens = s.tokens.map(t => idMap.get(t) || t);
        await this._client.spans.create(s.layerId, tokens, s.value, metaOrOmit(s.metadata));
      }
      for (const l of snapshot.vocabLinks) {
        const tokens = l.tokens.map(t => idMap.get(t) || t);
        await this._client.vocabLinks.create(l.vocabItemId, tokens);
      }
