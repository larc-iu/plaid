...
(defn- check-relation-invariants!
  "Check invariants for a relation between source and target spans.
  Validates that:
  - Both spans exist
  - Spans are in the same layer
  - Spans are in the same document  
  - Relation layer is linked to span layer"
  [db {:relation/keys [layer source target]} source-record target-record]
  ;; Source span exists?
  ...
  (when-not ((set (:span-layer/relation-layers (pxc/entity db (:span/layer source-record)))) layer)
  ...
  (when-not (= (get-doc-id-of-span db source) (get-doc-id-of-span db target))
...
(defn- check-relation-exists!
  "Check if a relation exists. Throws exception if not."
  [db eid]
...
(defn- check-relations-consistency!
  "Check if all relations belong to same layer and document.
...
(defn create* [xt-map attrs]
  (let [{:keys [db node] :as xt-map} (pxc/ensure-db xt-map)
    ...
    (when-not (:relation-layer/id (pxc/entity db layer))
      (throw (ex-info (pxc/err-msg-not-found "Relation layer" layer) {:id layer :code 400})))
    
    ;; Check all relation invariants
    (check-relation-invariants! db r source-record target-record)
  ...
  (let [{:keys [db]} (pxc/ensure-db xt-map)
    ...
    (op/make-operation
     {:type        :relation/update-attributes
      ...
      :tx-ops      (pxc/merge* xt-map :relation/id eid updates)})))
...
(defn set-end*
  ...
  (let [{:keys [db node] :as xt-map} (pxc/ensure-db xt-map)
        ...
        other-span-id (if (= :relation/target key) source target)
        other-span (pxc/entity db other-span-id)]
    
    ;; Relation exists?
    (check-relation-exists! db eid)
    ...
    ;; Check relation invariants for the new configuration
    (let [test-source (if (= :relation/source key) span-id source)
          test-target (if (= :relation/target key) span-id target)
          test-source-record (if (= :relation/source key) new-span (pxc/entity db source))
          test-target-record (if (= :relation/target key) new-span (pxc/entity db target))
          test-relation (assoc r :relation/source test-source :relation/target test-target)]
      (check-relation-invariants! db test-relation test-source-record test-target-record))
        ...
        (reduce
          (fn [tx-ops attrs]
            (let [relation-attrs (filter (fn [[k v]] (relation-attr? k)) attrs)
              ...
              ;; Check relation invariants
              (check-relation-invariants! db relation source-record target-record)
