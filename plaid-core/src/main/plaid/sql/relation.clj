...
(defn- validate-atomic-value!
  [value]
  (psc/validate-atomic-value! "Relation" value))
...
(def bulk-update-spec
  "The `plaid.sql.bulk/bulk-update!` spec for relations."
  {:table :relations
   :entity-type "relation"
   :layer-table :relation_layers
   :layer-key :relation_layer_id
   :op-type :relation/bulk-update
   :noun "relation"
   :values? true})

(defn bulk-update
  "Update the value of, and/or patch the metadata on, many relations in
  ONE operation. See `plaid.sql.bulk/bulk-update!` for the contract."
  [db items user-id]
