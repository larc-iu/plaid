...
;; Dead code path today — no REST route wires this. Kept for v2 parity in case a future endpoint needs it.
(defn remove-token
  "Drop a single token from a span's ordered token list. If the token
  was the span's only token, the span itself is deleted (relations
  referencing it are removed first, audited via psc/delete-by-id!).

  Returns the span id whether the span survived or was deleted (matches
  v2 return contract; v2's remove-token returned the submit-operations!
  result which carried the span id as the last put-docs / delete-docs
  target).

  Junction-table delete is paired with a single synthetic audit_writes
  row against :spans whose pre/post images carry the ordered token list
  under :tokens — mirrors the set-tokens pattern so ETL replay can
  reconstruct the new join state from one audit row representing the
  conceptual change."
  [db span-id token-id user-id]
  (let [pre (psc/fetch-by-id db :spans span-id)
        pre-tokens (when pre (fetch-token-ids db span-id))
        will-delete? (and pre
                          (= 1 (count pre-tokens))
                          (= token-id (first pre-tokens)))]
    (submit-operation!
     [tx db {:type :span/remove-token
             :project (project-id db span-id)
             :document (:document_id pre)
             ...
             :user user-id}]
     (let [span-row (psc/fetch-by-id tx :spans span-id)]
       (when (nil? span-row)
         (throw (ex-info (psc/err-msg-not-found "Span" span-id)
                         {:code 404 :id span-id})))
       (let [pre-tokens-tx (fetch-token-ids tx span-id)]
         (cond
           ;; Token isn't on the span — no-op. Mirrors v2's remove-join*
           ;; which silently produces an empty/no-change ops vector when
           ;; the value isn't present in the array column.
           (not (some #{token-id} pre-tokens-tx))
           span-id

           ;; Last surviving token — delete the span entirely (same
           ;; cascade as `delete`).
           (and (= 1 (count pre-tokens-tx))
                (= token-id (first pre-tokens-tx)))
           (let [rel-ids (get-relation-ids tx span-id)]
             (doseq [rid rel-ids]
               (psc/delete-by-id! tx :relations rid))
             (psc/delete-by-id! tx :spans span-id)
              ;; entity_metadata (no FK, manual sweep — same as `delete`).
             (psc/execute! tx
                           {:delete-from :entity_metadata
                            :where [:and
                                    [:= :entity_type "span"]
           ...
           ;; Trim case: remove the one junction row, emit one synthetic
           ;; audit row on the parent span carrying pre/post token lists.
           :else
           (do
             (psc/execute! tx {:delete-from :span_tokens
                               :where [:and
                                       [:= :span_id span-id]
                                       [:= :token_id token-id]]})
             (let [post-tokens (fetch-token-ids tx span-id)
                   pre-image (assoc span-row :tokens pre-tokens-tx)
                   post-image (assoc span-row :tokens post-tokens)]
