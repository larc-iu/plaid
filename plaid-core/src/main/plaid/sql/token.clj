...
(def bulk-update-spec
  ...
  column of its own (its surface is the slice of the text body it names),
  so a bulk update patches metadata only."
  {:table :tokens
   :entity-type "token"
   :layer-table :token_layers
   :layer-key :token_layer_id
   :op-type :token/bulk-update
   :noun "token"
   :values? false})

(defn bulk-update
  "Patch the metadata on many tokens in ONE operation. See
  `plaid.sql.bulk/bulk-update!` for the contract."
  [db items user-id]
