...
a predicate/field-path reads it as a number (so `["=", "?s.value", 5]` matches a
stored `5.0`), while the constraint map compares the stored text (`{"value": 5}`
does not). Stick to one form when comparing numeric annotation values.
