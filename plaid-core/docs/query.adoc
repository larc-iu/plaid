...
| `token`    | `layer`, `value`, `doc`, `begin`, `end`, `metadata`   | `value` is the token's surface text (the slice of the text body); `begin` / `end` are character offsets.
...
| `span`          | `value`, `doc`, `id`, `metadata.KEY…`
| `token`         | `value`, `begin`, `end`, `precedence`, `doc`, `id`, `metadata.KEY…`
| `relation`      | `value`, `doc`, `id`, `metadata.KEY…`
| `vocab`         | `form`, `id`, `metadata.KEY…`
