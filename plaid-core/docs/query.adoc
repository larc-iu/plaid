...
client = PlaidClient.login(
    "http://localhost:8080", "you@example.com", "password")
...
NOTE: For legibility, examples in this reference write a layer as a readable name
...
| `link`     | `item`, `doc`, `metadata`                             | A vocab link: the annotation that ties one or more tokens to a vocab item (several tokens make a multi-word expression). `item` is a vocab variable (a join) or an item id; its metadata carries provenance. Scoped through its document.
|===
...
* A plain string beginning with `?` is refused with a 400. Metadata is matched,
never bound, so a variable there could only ever read as a literal that no row
equals. To match a value that really does begin with `?` (a gloss like `?PL`),
wrap it: `{"literal": "?PL"}`, beside `{"regex": ...}`. The wrapper takes a
single value or a list of them, and means exactly what the bare form means.
...
["span", "?s", {"layer": "Gloss", "metadata": {"gloss": {"literal": "?PL"}}}]
----
...
spans, tokens, relations, vocab items, and links take part in these clauses;
documents and texts do not.
...
**Link entity.** When the link itself matters (who made it, whether anyone
confirmed it, how many tokens it covers), bind it as a `link` entity and walk to
its tokens and item: `["link-token", "?link", "?token"]` (`link`, `token`) and
`["link-item", "?link", "?vocab"]` (`link`, `vocab`). The `item` constraint on the
`link` clause is the inline form of `link-item`.

[,python]
----
# machine-made links nobody has confirmed, and the words they sit on
["link", "?l", {"metadata": {"prov": "inferred"}}],
["not", ["link", "?l", {"metadata": {"provConfirmed": True}}]],
["link-token", "?l", "?t"]

# every link to one lexicon entry, however many tokens each covers
["vocab", "?v", {"form": "kick the bucket"}], ["link-item", "?l", "?v"]
----
...
["=",  "?s.metadata.gloss", {"literal": "?PL"}]   # a literal that begins with ?
----
...
* On entity variables only `=` / `!=` are allowed (ids have no order); this is
  ...
  An entity or layer variable compares on its id, so the other term must be a
  variable, a field path, or an id: a bare name is a 400, since it could only
  ever match nothing. To compare an annotation's text, use its field path
  (`?s.value`) or constrain the clause.
* On value variables and literals, all six operators work.
* A term beginning with `?` reads as a variable. For a literal that really does
  begin with `?`, use the same `{"literal": …}` wrapper the
  <<Metadata,metadata constraint>> takes. It wraps one string, number, or
  boolean.
...
* `~` matches a **text** field (`value`, `form`, `name`, `body`, a
  `metadata`/`config` value, or a token's surface `value`) against a
...
element matches the sequence token directly. A `span` element takes `layer` /
`value` / `metadata`, a `token` element `value` / `begin` / `end` / `metadata`.

The `seq` config map takes `layer` (required) and an optional `doc` to pin the
whole sequence to one document. Those two belong to the config and nowhere else:
the config's `layer` is the token layer the sequence walks, so a `token`
element carrying its own `layer` is a 400, and so is a `doc` on an element of
either kind. (A `span` element's `layer` is its own span layer, which is a
different thing, and it is kept.)
...
narrow, never widen. If that intersection is empty (you scoped only to projects you
...
* When the pattern uses `or` / `seq`, a match is a binding of the variables
  EVERY alternative binds, so a row two alternatives both match counts once and
  a variable only one of them binds does not multiply it.
...
* Top-level and `scope` keys are recased to the wire format. Python
  `order_by` / `project_ids` and JavaScript `orderBy` / `projectIds` both become
  `order-by` / `project-ids` on the wire.
...
relation layer. Unlike the short examples above, these carry real layer ids, the
only form the engine accepts (see <<Layer addressing>>). Read them off the
project: `client.projects.get(project_id)` returns the layer tree with an `id` on
every layer.

[,python]
----
PROJECT   = "5b7ce985-9f11-4f2e-a1c0-6d1186cbd822"
SENTENCES = "0194e8a1-2c77-7d6b-9e30-4b1a5f0c8e11"   # token layer
MORPHEMES = "0194e8a1-2c77-7d6b-9e30-4b1a5f0c8e13"   # token layer
UPOS      = "0194e8a1-2c77-7d6b-9e30-4b1a5f0c8e21"   # span layer
DEPRELS   = "0194e8a1-2c77-7d6b-9e30-4b1a5f0c8e31"   # relation layer
...
client.query({
    "find": ["?r"],
    "where": [
        ["relation", "?r", {"layer": DEPRELS, "value": "nsubj", "target": "?dep"}],
        ["covers", "?dep", "?td"],
        ["within", "?td", "?s"],
        ["token", "?s", {"layer": SENTENCES}],
        ["first-in", "?td", "?s"],
    ],
})
...
# how many tokens each unconfirmed machine-made link covers (2+ = a multi-word expression)
client.query({
