...
So far a name has stood for an *entity*--a span, a token. A name can also stand for a *value*: write one where a condition's value would go, and instead of filtering, it remembers what it found, so that the same name elsewhere has to match.
This is how you ask for two things that agree without naming the value yourself--two words with the same lemma, say:

[,javascript]
----
where: [
  ["span", "?a", { layer: "lemma", value: "?v" }],
  ["span", "?b", { layer: "lemma", value: "?v" }],   // the same ?v -- so the same lemma
