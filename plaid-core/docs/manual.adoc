...
Each has a `title` (unique within the project, and the handle an assistant asks for one by, so a duplicate is a 409), a one-line `summary`, a Markdown `body`, and a `pinned` flag that an assistant reads as "send this one in full on every turn".
