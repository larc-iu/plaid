...
`tasks`:: A list of the slots this service fills, drawn from a small fixed vocabulary: `tokenize`, `parse`, `transcribe`, `link-vocab`, `analyze` (propose a morpheme segmentation and glosses for words), `translate` (propose a free translation for each sentence), `detect-speech` (propose time-aligned speech regions for a recording), `assist` (a conversational assistant that takes chat turns over a project). A client offering a "tokenize" action lists exactly the services whose `tasks` include `tokenize`.
