...
`bulk_update` exists for spans and relations (a value, a metadata patch, or both, per entry), for tokens (a metadata patch per entry) and for vocab items (a form, a metadata patch, or both, per entry):
