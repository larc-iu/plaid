...
`tasks`:: A list of the slots this service fills, drawn from a small fixed vocabulary: `tokenize`, `parse`, `transcribe`, `link-vocab`, `analyze` (propose a morpheme segmentation and glosses for words). A client offering a "tokenize" action lists exactly the services whose `tasks` include `tokenize`.
