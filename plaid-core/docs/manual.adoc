...
Each has a `title` (the handle an assistant asks for one by), a one-line `summary`, a Markdown `body`, and a `pinned` flag that an assistant reads as "send this one in full on every turn".
...
Writes ARE audited, so a change appears in the project's audit feed with pre- and post-images and announces on its event stream — but guidelines are project-scoped rather than document-scoped, so they are not time-travelable: `?as-of=` on these routes is a 400, not an answer.
...
`bulk_create` and `bulk_delete` exist for tokens, spans, relations and vocab links.
`bulk_update` exists for spans and relations (a value, a metadata patch, or both, per entry) and for tokens (a metadata patch per entry):

[,python]
----
client.spans.bulk_update([
    {"id": span_one, "value": "NOUN", "metadata": {"prov": "inferred"}},
    {"id": span_two, "metadata": {"provConfirmed": True}},
])
----

A `value` is set only when the key is present (`null` sets a null value); a `metadata` patch sets the keys it names and deletes a key whose value is `null`, leaving the rest alone.
...
[,js]
----
await client.documents.patchMetadata(docId, { plaid: { textDirection: "rtl" } });
----
