...
As for metadata, the canonical use is recording whether a human or a machine produced an annotation — Plaid standardizes this as the <<Provenance>> convention. App-private extras (e.g. a model's prediction probabilities) ride alongside the standard keys:
...
// Machine-made POS tag, not yet human-verified
{
  ...
  metadata: {
    ...
    // app-private extras are fine alongside the standard keys:
    tagProb: 0.8412
  }
}
----
