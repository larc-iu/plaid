...
export const ExportEditor = () => {
  ...
  const handleCopy = async () => {
    ...
    // permission rejects. Either way the button would keep saying "Copy" with
    // nothing said and an unhandled rejection in the console, and Download is
    // the only other way to get the text out.
    try {
  ...
  const handleDownload = () => {
    const blob = new Blob([umrContent], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = window.document.createElement('a');
    a.href = url;
    a.download = `${doc?.name || 'document'}.umr`;
