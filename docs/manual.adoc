...
In the previous section, you may have noticed that layers have restrictions: each layer type has exactly one other layer type which must serve as its parent.
Restrictions like these are necessary in order to keep Plaid efficient and easy to use, but in some cases, it may not be obvious 
