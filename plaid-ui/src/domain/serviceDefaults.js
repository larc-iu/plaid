...
// Stored in project config under the app's own namespace (`configNamespace`):
//
//   config.<app>.serviceDefaults = {
...
// online service).
//
// Selections are encoded as strings, shared by localStorage values and select
// option values:
//   'service:<serviceId>'  an external service
//   'builtin:<name>'       an app built-in implementation
//
// The built-in NAMES belong to the app that implements them, so each app keeps
// its own list of them.
...
export const readSpotDefault = (project, task) =>
  project?.config?.[configNamespace()]?.serviceDefaults?.[task] || null;
